...
1-Framework: FasTMCP 
(Hum ye wala frame work use krein gay. Jasey FasTAPI python ka frame work hai. Asey he FasTMCP MCP Server create krne ka frame work hai. Yani FatTAPI API bnane mein kam ata hai or FasTMCP MCP bnane mein kam ata hai.)
(MCP Server py he chlta hai. Yani jo bhi tools wgera hon gay wo server py he hon gay.)
...
(Hum tools main wo he chezein create karty/rakhty hain jo LLM ky pass nhi hoti. LLM ky pass real time data/personal 
data nhi hota. LLM ky pass Historical data hota hai. Yani LLM ky pass purana data hota hai. Historical data/purani chezon py hmara LLM train hai jbhi hum personal data/real time data provide kar rhy hain.)
(Hum ny python ka function bnaya wo hmara MCP Tool hai. Tool ka purpose kia hota hai Kisi bhi External Resources ko Communicate karna ho tw hum tools ka use krein gay.)
...
from fastmcp import FastMCP # (fastmcp ye package ka name hai or FastMCP ye class ka name hai)
                            # (Hum ny library ko import kia.Jasey Fastapi python ki library/framework hai jis main hum API likhty #  hain. Asey he FastMCP wo hai jis main hum apne MCP Server bnaty hain.)

mcp = FastMCP() # (mcp ka variable bnaya or phir FastMCP() class likhi.Class ka instance / Class ka object. Ye dono aik he bat 
                #  hai kuch bhi bolein.)
                # (Jasey hum FastAPI mein instance/object bnaty hain asey he FastMCP mein bhi apni application ka instance/object  #  bnaty hain.)

@mcp.tool()     # (Hum tools main wo he chezein create karty/rakhty hain jo LLM ky pass nhi hoti. LLM ky pass real time data/
                # personal data nhi hota.LLM ky pass Historical data hota hai. Yani LLM ky pass purana data hota hai. Historical
                # data/purani chezon py hmara LLM train hai jbhi hum personal data/real time data provide kar rhy hain.)
def get_ip_address():
...
from fastmcp import Client   # Hum fastmcp ki library/framework sy import kar ky client bna rahy hain
import asyncio
